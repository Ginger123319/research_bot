...
## 2026-03-20 会话（续二）：guoxue-72b-v1-2-reason 真实数据重评 + Skill 文档更新

### 📌 会话背景

上次 `xinghan-guoxue-72b-v1-2-reason` 的 QPS 评估使用了兜底数据（`_260313_260314.jsonl`），经与同事核验发现 system prompt 分布与真实线上数据不一致，需重新使用 AI-data 平台真实导出数据重评。

**新数据**：`/home/ceceadmin/wnd/workspace/repo/SpecForge/wnd_dev/data/downloaded/guoxue-v1-2-online-2026-01-1631-downloaded-s50k.jsonl`（50,000 条对话，1.9GB）

**新 SLA 要求**：E2E P90 < 180s，TTFS P90 < 1.5s（旧版 E2E P90 < 150s）

**服务端点**：`https://infer-test.geniuworks.com/bazi-guoxue-eagle3-test/v1/chat/completions`（无 API Key）

---

### ✅ 实现的功能

#### 1. 数据处理（traffic-dataset-prep，情况C 路径）

**问题背景**：下载的 JSONL 每行是 `list`（AI-data 平台 downloaded 格式），DataConverter 要求每行是 `dict`，直接调用报错。

**解决**：创建 `scripts/data/process_guoxue_v2.py`，绕过 DataConverter 和 DataSampler，自定义解析流程：

- 步骤0：探查模型别名 → 确认 `MODEL_LIST_SET = {"xinghan-guoxue-72b-v1-2-reason", "八字深度"}`
- 步骤1+1.5：逐行解析 list 格式，从 `role='b'.time`（ms→Asia/Shanghai）提取 `income_time`，从 `role='a'.prompt` 提取 `messages`，从 `role='a'.content` 提取 `old_response`
- 步骤2：pandas 直接过滤峰值窗口（替代 DataSampler）
- 步骤3~4：时间戳拼接 + DataInterpolator 泊松插值（TARGET_RPM=256）

**产出**：`datas/output_guoxue_v2/xinghan-guoxue-72b-v1-2-reason_selected_combined_2days_peak_poisson_256_stitched.csv`
...
#### 2. 服务探测（llm-service-probing）

无 API Key，使用 llm-service-probing Skill 探查端点，确认：
- 模型类型：对话模型（深度推理，长输出）
- 响应正常，E2E 延迟 ~300s（典型推理模型）

#### 3. 配置与脚本创建

创建以下文件：
- `configs/models/xinghan-guoxue-72b-v1-2-reason/8tp_v2.env`：SLA 参数（TTFS=1.5s，E2E=180s），COOLDOWN_SECS=900，DURATION=3600
- `scripts/benchmark/run_phase1_auto_guoxue8tp_v2.sh`：Phase 1 饱和探测
- `scripts/benchmark/run_phase2_auto_guoxue8tp_v2.sh`：Phase 2 自适应逼近（bracket 搜索）
- `scripts/benchmark/run_phase3_grid_guoxue8tp_v2.sh`：Phase 3 生产网格验证

#### 4. Phase 1 饱和探测（已完成）

| 并发档位 | decode_throughput | 增幅 |
|---------|-----------------|------|
| con=5 | ~180 tokens/s | — |
| con=10 | ~260 tokens/s | +44% |
| con=20 | ~330 tokens/s | +27% |
| con=30 | ~360 tokens/s | +9% |
| con=40 | ~362 tokens/s | +0.6% ✅ 饱和 |

**Phase 1 结论**：
```
peak_decode_throughput  = 360 tokens/s
avg_output_len          = 1386 tokens
...
检查点写入：`logs/guoxue-v2-phase1_20260320_1524/phase1_checkpoint.md`

#### 5. Phase 2 自适应逼近（运行中）

初始参数纠正后以 `INIT_LO=0.200, INIT_HI=0.312` 启动：

| 轮次 | RPS | 结果 | bracket 更新 |
|------|-----|------|------------|
| 第1轮 | 0.2498 | ❌ FAIL（E2E P90 超标）| HI→0.2498 |
| 第2轮 | 0.2235 | ❌ FAIL | HI→0.2235 |
| 第3轮 | 0.2114 | 运行中→后续轮次继续 | LO=0.200, HI=0.2235 |

**会话结束时 Phase 2 状态**：后台运行中，每档 3600s（1h），档位间冷却 900s（15min）

#### 6. Skill 文档更新（本次新增）

**traffic-dataset-prep/SKILL.md**：
- 新增「情况C：AI-data 平台 downloaded 导出格式」完整路径（跳过 DataConverter/DataSampler）
- 新增步骤0（别名探查）
- 新增 pandas 直接解析代码模板
- DATA_FORMAT 枚举新增 `downloaded` 值
- 常见错误表新增 2 条（`AttributeError: 'list' object has no attribute 'get'`、`KeyError: 'file_suffix'`）
- 验证 Checklist 新增格式判断入口行

**qps-peak-finder/SKILL.md**：
- 新增「Phase 0 产出：DURATION / COOLDOWN 自动推算」节，带完整公式：
  - `DURATION_SECS = max(1200, int(avg_output_len × 2.5))`
  - `COOLDOWN_SECS = max(60, int(avg_output_len / 2))`
- 新增测试污染警告（长输出推理模型专项）
- Phase 2 新增 `production_rps` 配置陷阱说明
- 常见问题表新增测试污染和 production_rps 误配两条

---

### 🐛 遇到的错误 & 解决方案

#### 错误1：`AttributeError: 'list' object has no attribute 'get'`（DataConverter）
- **现象**：`process_guoxue_v2.py` 调用 DataConverter 时报错
- **根因**：`downloaded-s50k.jsonl` 每行是 `list`，DataConverter 期望 `dict`
- **解决**：完全绕过 DataConverter，自定义 pandas 解析逻辑

#### 错误2：DataSampler 依赖 `file_suffix` / `original_index` 列
- **现象**：自定义 CSV 缺少 DataConverter 的索引列，DataSampler 报 `KeyError`
- **解决**：移除 DataSampler 调用，直接用 pandas 时间窗口过滤

#### 错误3：`NameError: name 'GlobalConfig' is not defined`
- **现象**：步骤4 DataInterpolator 报错
- **根因**：从 DataConverter 块中删除了 `GlobalConfig` 的 import
- **解决**：在步骤4 前显式加 `from scripts.data_processor.config.settings import GlobalConfig`

#### 错误4：Phase 1 脚本 grep 模式不匹配
- **现象**：脚本无法提取 `decode_throughput`，饱和判断始终走 `+10` 分支
- **根因**：grep 模式为 `decode_throughput=\K[\d.]+`，但 analyze 输出格式为 `decode_throughput : value`
- **解决**：改为 `decode_throughput\s*[=:]\s*\K[\d.]+` 兼容两种分隔符

#### 错误5：Phase 2 `PRODUCTION_RPS` 误配 + 测试污染
- **现象**：低 RPS 档反而比高 RPS 档延迟更高（bracket HI 单调递减，不收敛）
- **根因1**：`PRODUCTION_RPS=4.27`（从 Poisson 目标 RPM/60 错误换算），导致 bracket 搜索过早托底
- **根因2**：`COOLDOWN_SECS=60` 严重不足（模型 E2E P90 ~300s），上一档在途请求积压进入下一档
- **解决**：`PRODUCTION_RPS` 改为 `0.05`（安全下限），`DURATION_SECS`→3600，`COOLDOWN_SECS`→900，终止重启 Phase 2
...
## 2026-03-20 会话结束总结（本次会话：chart-32b 回放结果 + 评估报告生成）

### 📌 会话概览

- **会话日期**：2026-03-20（下午至傍晚）
- **衔接上次**：上次会话已完成 skill 文档更新（traffic-dataset-prep/model-evaluation-workflow/model-onboarding.md）；chart-32b 回放测试在 14:52 已在后台启动
- **本次目标**：① 确认回放测试已完成并分析结果 ② 生成 chart-32b EVAL_REPORT.md ③ 全链路归档

---

### ✅ 实现的功能

#### 1. chart-32b 回放测试结果分析（Step 6）

**回放已在后台完成**（14:52→16:43，110分钟），本次会话拉取结果：

| 指标 | 值 | SLA | 判断 |
|------|-----|-----|------|
| 成功率 | 99.19% | ≥99% | ✅ 宽松通过 |
| TTFT P90 | 0.135 s | ≤1.5s | ✅ 余量91% |
| TTFS P90 | 0.442 s | ≤1.5s | ✅ 余量71% |
| E2E P90 | 6.811 s | ≤150s | ✅ 极宽裕 |

- 总请求 5,826 条，47 条失败（0.81%），偶发超时，非容量问题
- 单实例均摊约 39 RPM（keep-income-time 模式下平均），离 ideal 457 RPM 还有 11× 余量

**新增文件**：
- `results/chart-32b_replay_20260320/REPORT.md`（6节完整报告）
- `results/chart-32b_replay_20260320/chart-32b-8tp_replay_118rpm_analysis.html` + 7张PNG图

**更新文件**：
- `results/models/INDEX.yaml`：replay_* 字段填入，recommendation 更新为 ✅ 可上线
- `results/models/xinghan-chart-32b-v1-1-agent/model-context.md`：追加回放结果字段
- `results/README.md`：新增 chart-32b_replay_20260320 条目 + 详细节

#### 2. chart-32b EVAL_REPORT.md 生成（Step 7）

**调用 Skill**：model-eval-report（按 model-evaluation-workflow Step 7）

**资源计算结果**：
```
业务峰值：118 RPM = 1.967 req/s
ideal_rps（8TP）：7.6132 req/s = 457 RPM
单实例覆盖率：387%（极宽裕）
最低实例数：1（含10%余量仍覆盖349%）
HA 配置建议：2 实例 × 8TP = 16 卡 L20
```

**上线判断**：✅ 可上线（成功率 99.19% ≥ 99%，延迟全部通过，QPS容量11×余量）

**新增文件**：
- `results/models/xinghan-chart-32b-v1-1-agent/EVAL_REPORT.md`（两层结构：摘要 + 技术详情 + 资源规划参考表）

**更新文件**：
- `results/models/INDEX.yaml`：`eval_report` 路径填入，notes 更新（移除旧版 ⚠️ 警告）

---

### 🐛 遇到的问题

**无新错误**：本次会话均为已完成实验的结果分析归档，无需调试。
...
| 文件 | 操作 | 说明 |
|------|------|------|
| `results/chart-32b_replay_20260320/REPORT.md` | 新增 | 回放测试详细报告 |
| `results/models/xinghan-chart-32b-v1-1-agent/EVAL_REPORT.md` | 新增 | 最终交付评估报告 |
| `results/models/INDEX.yaml` | 修改 | replay_* / eval_report / recommendation 字段 |
| `results/models/xinghan-chart-32b-v1-1-agent/model-context.md` | 追加 | 回放测试结果字段 |
| `results/README.md` | 修改 | 新增 chart-32b_replay 条目 |
| `progress.md` | 追加 | 本次会话记录 |

---

### 🎯 技术决策

1. **indexed格式 Agent 数据纳入回放**：通过 _full.csv 分片+DataSampler 方式将 87% 的 Agent 框架调用纳入回放数据集，确保流量代表性
2. **资源建议采用公式+HA双维度**：单实例 387% 余量超大，直接推荐最小配置（1实例）并注明 HA 场景建议（2实例），避免超配

---

### 🏁 xinghan-chart-32b-v1-1-agent 全流程完成状态

| 步骤 | 状态 | 完成日期 |
|------|------|---------|
| Step 0 信息收集 | ✅ | 2026-03-18 |
| Step 3 数据处理（indexed格式）| ✅ | 2026-03-19~20 |
| Step 4 远端连通性 | ✅ | 2026-03-18 |
| Step 5 QPS peak-finder（8TP+4TP）| ✅ | 2026-03-19 |
| Step 5 回放测试（118 RPM）| ✅ | 2026-03-20 |
| Step 6 结果分析归档 | ✅ | 2026-03-20 |
| Step 7 EVAL_REPORT.md | ✅ | 2026-03-20 |

---

## 2026-03-20 会话（续）：guoxue-72b V2 × 4×H20 QPS Peak Finder

### 背景
用户在 4×H20 部署了 `xinghan-guoxue-72b-v1-2-reason`，endpoint：
`https://infer.geniuworks.com/infra-xinghan-guoxue-p72b-v12-reason/v1/chat/completions`
V2 真实线上数据集已就绪，直接从 Step 4 远端连通性开始评估。

### Step 4 ✅ 远端连通性验证（2026-03-20）
- endpoint：`https://infer.geniuworks.com/infra-xinghan-guoxue-p72b-v12-reason/v1/chat/completions`
- 服务模型 ID：`infra-xinghan-guoxue-p72b-v12-reason`
- max_model_len：8192
- 基线延迟：0.98s（max_tokens=32）
- 服务探测：通用对话模型（推理增强型，含 <think> 链）✅
- 多轮上下文：✅ 正常
- 结构化输出：✅ 支持

### 新建文件
- `configs/models/xinghan-guoxue-72b-v1-2-reason/4h20.env`（H20 配置）
- `scripts/benchmark/run_phase1_auto_guoxue4h20.sh`（Phase 1 脚本）
- `scripts/benchmark/run_phase2_auto_guoxue4h20.sh`（Phase 2 脚本）
- `scripts/benchmark/run_phase3_grid_guoxue4h20.sh`（Phase 3 脚本）
- `results/models/xinghan-guoxue-72b-v1-2-reason/model-context.md` 已追加 H20 段落

### Step 5 🔄 Phase 1 饱和探测（2026-03-20 启动）
**先验信息**：H20 单卡吞吐 = 5× L20，4H20 vs 8L20 总吞吐 ≈ 2.5×
- L20 8TP 饱和点 con=30 → H20 4TP 预期饱和 ~75，从 **con=60** 起跳
- 每档时长：300s（5min）
- 日志目录：`logs/guoxue-v2-h20-phase1_20260320_2023/`
- Phase 1 进行中，后续 Phase 2（1h/档+15min 冷却）→ Phase 3（4档×1h）自动衔接

---

## 🗓️ 会话记录 — 2026-03-23（下午场）：V1 数据归档 & 资产清理

### 📌 会话目标
`xinghan-guoxue-72b-v1-2-reason` 历史评估资产清理：将 V1 兜底数据实验目录归档，基于 V2 真实数据重写最终评估报告 `EVAL_REPORT.md`，更新 INDEX.yaml 和 Dashboard 展示。

### 问题背景
- 2026-03-16~18 的实验（`guoxue_qps_sweep_20260317`、`guoxue_qps_peak_finder_20260318` 等）使用了 **V1 兜底数据**（system prompt 不对）
- 更严重的问题：业务峰值口径只统计了 stream 43 RPM，**漏统 MCP 八字深度 213 RPM**，导致实例规模建议严重偏低（4 实例 → 实际需 ≥9 实例）
- 2026-03-20~23 基于 V2 真实数据、正确的 256 RPM 峰值口径完成了全套重测

### ✅ 实现了哪些功能

#### 1. 旧 V1 实验目录归档
- 物理移动 4 个目录到 `results/archive/guoxue-v1-deprecated/`：
...
- 创建 `results/archive/guoxue-v1-deprecated/ARCHIVED.md`（说明废弃原因、V1 vs V2 差异、替代实验指针）

#### 2. results/README.md 更新
- 目录总览表中旧 guoxue 条目标注 *(已归档，V1 兜底数据)*，添加新 V2 三条目
- 详细说明区：旧两个 guoxue 节标为已归档并说明原因，新增 `guoxue-v2-8tp-peak-finder-20260323`、`guoxue-v2-h20-peak-finder-20260323`、`guoxue-v2-h20-vs-l20-peak-finder-20260323` 三节详细说明

#### 3. model-context.md 更新
- Step 3 数据处理部分：`dataset_path` 切换为 V2 路径，峰值 220→256 RPM，行数 5726→6358
- 删除 V1 QPS 结论字段，保留为注释
- 回放状态从"跳过"改为"进行中（2026-03-23 启动）"

#### 4. EVAL_REPORT.md 重写（V2 版本）
- 主要结论变更：4×H20 为推荐方案（ideal_rps=0.5878，35.3 RPM）
- 业务峰值：256 RPM（修正）
- 推荐规模：9 实例 × 4H20 = **36 卡 H20**（vs 旧报告的 4 实例 × 8L20 = 32 卡 L20）
- 包含 H20 vs L20 完整对比（H20 以 0.5× GPU 实现 2.78× QPS，E2E -25%，TTFS -28%）
- 回放测试结果留占位符（进行中）

#### 5. INDEX.yaml 更新
- `deployment` 主配置改为 4×H20
- `performance` 数值更新为 H20 结果（sla_max_qps_rps: 0.5878，saturation_rps: 0.7348 等）
- `recommendation` 更新为 H20 优先建议
- `linked_experiments` 清理：移除 V1 实验链接，改为注释；保留 4 个 V2 有效链接

#### 6. Dashboard 展示（serve.py @ :18999）
- `serve.py` 直接读取 INDEX.yaml，无需重启，更改立即生效
- guoxue 模型卡片现在展示 H20 配置和新的推荐建议

### ⚠️ 未完成（待回放结果）
- 回放测试于 2026-03-23 启动，预计约 2 小时后完成
- 完成后需补充 `EVAL_REPORT.md` 第 3 节（回放压测结论）和 `model-context.md` replay_* 字段及 `INDEX.yaml` replay_* 字段

### 遇到的错误
- `StrReplace` 对含全角标点的中文行匹配失败 → 改用 Python 脚本直接替换（`content.replace(old, new)`）解决

---

## 2026-03-23 会话（续）：guoxue-72b 4H20+9L20 回放测试完成 & 结果分析

### 📌 背景

上次会话已在后台启动 `4×4H20 + 9×8L20` 新方案的回放测试（PID 168511，17:26 启动）。
本次会话确认测试已完成，并完成结果分析、REPORT.md 撰写及文档归档。

### ✅ 实现了哪些功能

#### 1. 确认回放测试完成

通过 `ps -ef | grep replay` 确认回放进程已结束，查看日志：
- 总样本：6,358 条，成功 6,358 条，**成功率 100%**（benchmark 工具层面）
- 测试时长：17:26 → 18:20，约 54 分钟

#### 2. 离线统计分析（offline_analysis.py）

启动 PID 214183，处理 6.3GB CSV，内存峰值约 28GB，运行约 5 分钟完成。

产出文件（`results/guoxue-v2-4h20-9l20-replay-20260323/`）：
- `guoxue-4h20-9l20_replay_256rpm_analysis.html`（2.9MB 交互式报告）
...
#### 3. 撰写 REPORT.md（含两组方案横向对比）

新建 `results/guoxue-v2-4h20-9l20-replay-20260323/REPORT.md`，包含：
- 部署背景（释放 2 台 H20 → 补入 5 台 L20 的动机）
- 测试参数（复用基线 256 RPM 数据集）
- 完整指标（TTFT/TTFS/E2E 各分位数）
- **与基线方案（6H20+4L20）横向对比表**
- 容量分析（对三月峰值 195 RPM 余量 +31%）
- 风险提示（E2E P99=148.8s 接近 SLA=180s 的 83%）

#### 4. 横向对比结论

| 指标 | 原方案 6H20+4L20 | 新方案 4H20+9L20 | 变化 | SLA |
|------|:--------------:|:------------------:|:----:|:---:|
| 成功率 | 99.89% | **99.89%** | = | ✅ |
| TTFT P90 | 0.989 s | **1.029 s** | +4.0% | ✅ |
| TTFS P90 | 1.139 s | **1.182 s** | +3.8% | ✅ |
| E2E P90 | 100.7 s | **115.5 s** | +14.7% | ✅ |
| E2E P99 | 121.2 s | **148.8 s** | +22.8% | ✅ |

延迟小幅升高属预期（L20 份额从 40% 升至 69%），SLA 全通过，**新方案可投入生产**。

#### 5. 文档归档

| 文件 | 操作 |
|------|------|
| `results/guoxue-v2-4h20-9l20-replay-20260323/REPORT.md` | 新建 |
| `results/README.md` | 新增条目（目录表 + 详细节）|
| `results/models/xinghan-guoxue-72b-v1-2-reason/model-context.md` | 追加 replay2_* 字段 |
| `results/models/INDEX.yaml` | 追加 replay2_* 字段 + linked_experiments 条目 |

### 遇到的错误

- `offline_analysis.py` 使用 `usecols` 读取 CSV 失败（列名为 raw token_list 格式，不含 ttft_s/e2e_s）→ 让分析脚本自行运行，从 log 输出提取各分位数指标

---

## 2026-03-22 会话：xinghan-guoxue-72b-v1-2-reason 4×H20 QPS Peak Finder 全流程

### 📌 背景

xinghan-guoxue-72b-v1-2-reason 部署在 4×H20 新机型（endpoint: `https://infer.geniuworks.com/infra-xinghan-guoxue-p72b-v12-reason/v1/chat/completions`），需评估其 SLA 合规容量（TTFS P90 ≤ 1.5s，E2E P90 ≤ 180s）。继上次会话已完成 Phase 1 启动并修复 num_requests bug 后，本次会话完成了 Phase 1 完整结果分析、Phase 2 全部收敛、Phase 3 启动，同时修复了 Phase 1 脚本中的另一处严重 bug。

### ✅ 实现了哪些功能

#### 1. Phase 1 饱和探测完成（con=60 → con=120）

| 档次 | decode_throughput | 成功请求 | 饱和判断 |
|------|------------------|---------|---------|
| con=60 | **1056.724 tokens/s** | 186/187 | 峰值档 ✅ |
| con=120 | 694.335 tokens/s | 129/2400 | 过载（-34.3%，2270 超时）|

- 饱和点在 con=60~120 之间；峰值在 con=60
- max_rps_estimate（峰值）= 1056.724 / 2157.3 = **0.4898 req/s**
- avg_output_len 实测 = **2157.3 tokens**（高于 Phase 0 估算 1386 tokens，推理模型生成更长的思考链）
- H20 4 卡吞吐（1056 tokens/s）vs L20 8 卡（约 360 tokens/s）= 实际提升 **2.9×**（理论预期 2.5×，略优）

#### 2. 修复 Phase 1 脚本两处 bug

**Bug A：max_rps_estimate 正则错误**

分析脚本输出格式：`max_rps_estimate = 694.335 / 2177.3 = 0.3189 req/s`

- 旧正则：`grep -oP 'max_rps_estimate\s*[=:]\s*\K[\d.]+'` → 匹配到第一个数字 `694.335`（tokens/s），而非最终值 `0.3189`（req/s）
- 导致检查点写入 `max_rps_estimate = 694.335 req/s`、`START_RPS = 833.202` 完全错误
- 新正则：`grep -oP 'max_rps_estimate\s*=\s*[\d.]+\s*/\s*[\d.]+\s*=\s*\K[\d.]+'` ✅

**Bug B：饱和时使用过载档指标而非峰值档**

- 旧逻辑：当吞吐下降检测到饱和时，直接用当前档（已过载）的 max_rps 写入检查点
- 新逻辑：全程追踪 `PEAK_THROUGHPUT` / `PEAK_MAX_RPS` / `PEAK_CON`，饱和时汇报峰值档数据

修改文件：
- `scripts/benchmark/run_phase1_auto_guoxue4h20.sh`（新增 PEAK_* 变量追踪，修复正则，修复饱和确认块）
- `logs/guoxue-v2-h20-phase1_20260320_2030/phase1_checkpoint.md`（人工写入正确值）

#### 3. Phase 2 自适应逼近完成（5 档，全部一次收敛）

| 档次 | RPS | E2E P90 | TTFS P90 | SLA | 操作 |
|------|-----|---------|----------|-----|------|
| 第1档 | **0.5878** | **114.83s** ✅ | 636ms ✅ | PASS | LO=0.5878，大步 ×1.25 |
| 第2档 | 0.7348 | 384.29s ❌ | 1031ms ✅ | FAIL | HI=0.7348 |
| 第3档 | 0.6572 | 192.76s ❌ | 750ms ✅ | FAIL | HI=0.6572 |
| 第4档 | 0.6215 | 186.34s ❌ | 744ms ✅ | FAIL | HI=0.6215 |
| 第5档 | 0.6044 | 182.07s ❌ | 742ms ✅ | FAIL | bracket 宽度 **2.82%** → 收敛 |

**Little's Law 验证（内部一致性）**：
- rps0.5878：理论并发 = 0.5878 × 114.83 = 67.5 ≈ max_active=65 ✅
- rps0.6044：理论并发 = 0.6044 × 182.07 = 110 ≈ max_active=106 ✅

**结论**：**ideal_rps = 0.5878 req/s = 35.3 RPM**（bracket 宽度 2.82%，精度高）

注意事项：
- 陡崖现象：0.5878→0.7348（+25% RPS）导致 E2E P90 从 114s 暴增至 384s，系统接近 M/M/c 饱和拐点
- 首个失败点 0.6044 仅超标 1.15%，SLA 边界非常精确

用时：约 9 小时（2026-03-20 21:59 → 2026-03-21 03:34），每档 1h + 冷却 15min

#### 4. Phase 3 验证网格启动

- 4 档均匀分布于 [0.05, 0.5878]：**0.0500 / 0.2293 / 0.4085 / 0.5878** req/s
- 每档 3600s + 档间冷却 900s，预计总时长约 **5 小时**
- 已于 2026-03-22 17:59 启动，PID 3540380
- 日志：`logs/data-pipeline/guoxue_v2_h20_phase3_20260322_175900.log`
- 输出：`logs/guoxue-v2-h20-phase3_20260322_1759/`

### 🐛 遇到的错误及解决方式

| 错误 | 原因 | 解决方式 |
|------|------|---------|
| Phase 1 检查点 max_rps=694.335 req/s（数量级完全错误）| 正则误匹配 decode_throughput 数值 | 修改正则为三段式匹配，捕获最终 req/s 值 |
| 饱和检查点引用过载档而非峰值档 | 脚本只保存当前档指标，未追踪历史峰值 | 新增 PEAK_* 变量，全程比较并更新 |
| Phase 2 启动时未传环境变量 | nohup 启动时 PEAK_RPS/START_RPS 未导出 | kill 重启，改为 `PEAK_RPS=0.4898 START_RPS=0.5878 nohup bash ...` |
| CSV field_size_limit 超限 | 推理模型输出极长，raw_response 字段超 131072 | Python `csv.field_size_limit(10**7)` |
| 后台分析脚本超时（5 min/档 CSV，共 5 档）| 大型 CSV（2.3 GB/档）tokenize 统计慢 | 分档后台分析，直接从 Phase 2 日志提取关键指标 |

### 📁 本次新增/修改文件

| 文件 | 操作 |
|------|------|
| `scripts/benchmark/run_phase1_auto_guoxue4h20.sh` | 修改（正则、PEAK 追踪、饱和确认块）|
| `logs/guoxue-v2-h20-phase1_20260320_2030/phase1_checkpoint.md` | 人工重写（正确值）|
| `logs/guoxue-v2-h20-phase2_20260320_2053/phase2_checkpoint.md` | 自动生成（脚本产出，正确）|

### ⚠️ 未完成（下次会话跟进）

- Phase 3 正在运行（4档，约 17:59 → 23:00 预计完成）
- Phase 3 完成后需运行 `qps-peak-finder-analysis` Skill 生成 REPORT.md
- 更新 `results/models/xinghan-guoxue-72b-v1-2-reason/model-context.md` 中 H20 评估字段（h20_ideal_rps, h20_ideal_rpm, h20_phase3_done 等）

---

## 📋 2026-03-22（续）— Phase 2 收尾 + Phase 3 启动 + 自动衔接健壮性修复

### ✅ 实现的功能

#### 1. 诊断 L20 Phase 2 脚本异常退出原因

检查 `logs/guoxue-v2-phase2_20260320_1920/` 各轮结果，发现：

| 轮次 | RPS | E2E P90 | TTFS P90 | SLA |
|------|-----|---------|---------|-----|
| Round 1 | 0.2498 | 475.09s | — | ❌ FAIL |
| Round 2 | 0.2235 | 418.63s | — | ❌ FAIL |
| Round 3 | 0.2114 | **153.19s** | **885ms** | ✅ PASS（裕量17.5%）|
| Round 4 | 0.2174 | — | — | 未执行（目录存在但为空）|

**根因**：Phase 2 脚本在 Round 4 准备阶段意外崩溃，`phase2_checkpoint.md` 从未写入。Phase 3 衔接脚本 `run_phase3_auto_guoxue8tp_v2.sh` 等待 24 小时后超时退出，Phase 3 始终未触发。

Bracket 状态：`[LO=0.2114, HI=0.2235]`，宽度 5.4%（接近 5% 阈值）。保守取 PASS 下界 `ideal_rps = 0.2114 req/s`。

#### 2. 手动写入 L20 Phase 2 checkpoint

补写文件：`logs/guoxue-v2-phase2_20260320_1920/phase2_checkpoint.md`

| 指标 | 值 |
|------|-----|
| ideal_rps | **0.2114 req/s** |
| ideal_rpm | **12.7 RPM** |
| 裕量 | 17.5%（E2E P90=153.19s vs 限制 180s）|
| max_active_requests | 32（服务稳定）|
| decode_throughput | 455 tok/s |

#### 3. 同时启动 L20 + H20 Phase 3 网格（2026-03-22 17:54）

两个端点完全独立，可并行：

| 部署 | ideal_rps | 4 档网格 | 输出目录 |
|------|-----------|---------|---------|
| **L20 (8tp)** | 0.2114 | 0.0500 / 0.1038 / 0.1576 / 0.2114 req/s | `logs/guoxue-v2-phase3_20260322_1754/` |
| **H20 (4h20)** | 0.5878 | 0.0500 / 0.2293 / 0.4085 / 0.5878 req/s | `logs/guoxue-v2-h20-phase3_20260322_1754/` |

每档 1h 测试 + 15min 冷却 = 4 × 75min = **约 5 小时**，预计 ~22:54 完成。

#### 4. Brainstorming 设计 + 实现 Phase 2→3 自动衔接健壮性双保险

**根因分析**：Phase 3 衔接脚本的唯一触发条件是 `phase2_checkpoint.md` 存在。一旦 Phase 2 进程在 checkpoint 写入前崩溃，衔接脚本就永远等待直至 24h 超时。

**设计方案（经 Brainstorming 三轮确认）**：

- **方向 C**：脚本修复 + Skill 文档更新双管齐下
- **Phase 2 策略 C**：`trap EXIT` 兜底 + 衔接脚本自主推断双保险
- **Skill 文档**：只更新 `qps-peak-finder` skill

**实现细节**：

*Phase 2 脚本（L20 + H20 各自修改）*：

1. `LAST_PASS_RPS=""` 初始化，LO 更新时同步赋值
2. `write_exit_checkpoint()` 函数：
   - 正常收敛 → checkpoint 已存在，直接返回
   - 意外退出 + 有 PASS → 写 `[EXIT_TRAP]` checkpoint（以 `LAST_PASS_RPS` 为 `ideal_rps`）
   - 意外退出 + 无 PASS → 写 `phase2_no_pass.md`
3. `trap write_exit_checkpoint EXIT` 注册（`OUTPUT_BASE` 确定后立即注册）
4. Production floor 分支（原本只打印命令后 `break`）：补写 `[PRODUCTION_FLOOR]` checkpoint

*Phase 3 衔接脚本*：

替换旧"方式2"（错误地用目录数量 ≥20 判断），新逻辑：

```
第 ≥3 次轮询时：
  if Phase2 进程名不在 ps aux → 进程已死
     逐一调 analyze_peak_finder.py 分析已有 rps* 目录
     取最高 PASS RPS 作为 ideal_rps
     写入 phase2_recovery_checkpoint.md [RECOVERY]
     进入 Phase 3
  else → 继续等待
```

新增变量：`ANALYZE`、`TTFS/E2E_P90_LIMIT`、`PHASE2_SCRIPT_NAME`

*`qps-peak-finder` Skill 文档*：

- Phase 2 执行说明增加"脚本健壮性（EXIT trap 机制）"说明段，解释 4 种 checkpoint 来源标记：`normal` / `[EXIT_TRAP]` / `[PRODUCTION_FLOOR]` / `[RECOVERY]`
- 常见问题表新增行：Phase 2 有 `rps*/` 但无 checkpoint 的场景 + 处理方式

### 🐛 遇到的错误及解决方式

| 错误 | 原因 | 解决方式 |
|------|------|---------|
| Phase 3 衔接脚本等 24h 超时退出，Phase 3 从未启动 | Phase 2 脚本在 Round 4 准备时崩溃，checkpoint 未写入；衔接脚本单点依赖 checkpoint 文件 | 诊断后手动补写 checkpoint，同时实现双保险机制防止再发 |
| H20 Phase 3 也未启动（虽然 Phase 2 已收敛） | H20 没有对应的 Phase 3 衔接脚本，需手动触发 | 直接带参数启动 `run_phase3_grid_guoxue4h20.sh` |
| 旧"方式2"用 `grep -l 'SLA_PASS' CSV` 判断 PASS | SLA_PASS 是 `analyze_peak_finder.py` 的输出，不在 CSV 文件内容里 | 替换为调用 `analyze_peak_finder.py` 的正确逻辑 |

### 📁 本次新增/修改文件

| 文件 | 操作 | 说明 |
|------|------|------|
| `logs/guoxue-v2-phase2_20260320_1920/phase2_checkpoint.md` | 新增（手动写入）| L20 Phase 2 收敛补档 |
| `scripts/benchmark/run_phase2_auto_guoxue8tp_v2.sh` | 修改 | trap EXIT + LAST_PASS_RPS + production floor checkpoint |
| `scripts/benchmark/run_phase2_auto_guoxue4h20.sh` | 修改 | 同步 trap EXIT 机制 |
| `scripts/benchmark/run_phase3_auto_guoxue8tp_v2.sh` | 修改 | 新增 ANALYZE 变量 + 进程死亡检测 + analyze 自主推断方式2 |
| `clingo/docs/skills/qps-peak-finder/SKILL.md` | 修改 | 新增 EXIT_TRAP 健壮性说明段 + 常见问题新增行 |

### ⚠️ 未完成（下次会话跟进）

- L20 + H20 Phase 3 正在运行（约 17:54 → 22:54 预计完成）
- Phase 3 完成后：
  - 运行 `qps-peak-finder-analysis` Skill 分别生成 L20 和 H20 的 REPORT.md
  - 合并两份报告写入 `results/models/xinghan-guoxue-72b-v1-2-reason/` 目录
  - 更新 `model-context.md` 中 L20 和 H20 各字段
- 回放测试（guoxue 模型尚未执行 replay benchmark）


---

## 📋 会话记录 — 2026-03-23（运维 + Dashboard 配置）

### ✅ 实现的功能

#### 1. 🔧 重启 serve.py 监控服务

- **背景**：`serve.py` 原进程（PID 1890349）因升级 SGLang 版本等操作后无法访问，需重启
- **操作**：
  - kill 旧进程 PID 1890349
  - 在 `/mnt/ai-infra/users/wnd/workspace/execute/guofan` 目录下后台重启
  - 日志写入 `logs/data-pipeline/serve_18999.log`
- **新进程 PID**：4041588
- **命令**：
  ```bash
  ...
  ```
- **验证**：`ps -ef | grep serve.py` 确认进程存活

#### 2. ✨ xinghan-ziwei-32b-v1 在线承载量监控接入

- **背景**：上周将 xinghan-ziwei-32b-v1 的 SGLang 从 v0.4.1 升级到 v0.4.6，Grafana Dashboard 对应新部署名 `xinghan-ziwei-p32b-v1-sglang046`；旧 INDEX.yaml 未配置 `online_deployment_name`，Dashboard 无在线 RPM 展示
- **操作**：在 `results/models/INDEX.yaml` 的 `xinghan-ziwei-32b-v1` 条目新增：
  ```yaml
  online_deployment_name: xinghan-ziwei-p32b-v1-sglang046
  ```
- **验证**：查询 VictoriaMetrics 确认 `sglang:num_requests_total`（v0.4.6 新格式）有数据
  - 当前 RPM：~13~16 RPM
  - 7 日峰值：52 RPM（2026-03-22）
  - 有效天数：3 月 21~23 日（v0.4.6 升级后才有记录）

#### 3. ✨ xinghan-ziwei-32b-v1 实例数修正 + 缩容建议

- **背景**：实际为 2 实例部署，但 INDEX.yaml 未填写 `current_instances`，Dashboard 默认按 1 实例计算 SLA 对比（显示 55%），实际应为 2×95.4=190.8 RPM
- **操作**：
  - 新增 `current_instances: 2` 到 deployment 字段
  - 更新 recommendation 为缩容建议
- **修改后 recommendation**：
  > ✅ 当前2实例联合承载 190.8 RPM（余量 3×），7日实测峰值仅 52 RPM（占用 27%）；建议缩容至 1 实例×8TP，可节省 8 卡 L20，单实例上限 95.4 RPM 仍有 83% 余量
- **验证**：`/api/online-rpm` 接口返回 current_rpm=16 RPM，SLA 对比显示 27%（绿色）

### 🐛 遇到的错误及解决方式

| 错误 | 原因 | 解决方式 |
|------|------|---------|
| serve.py 服务无法访问 | 旧进程（PID 1890349）运行异常 | kill 旧进程，`nohup` 后台重启，日志重定向至 `logs/data-pipeline/serve_18999.log` |
| Dashboard 在线 RPM 无展示 | `online_deployment_name` 字段缺失 | 补填 `xinghan-ziwei-p32b-v1-sglang046` |
| SLA 对比显示 55% 而非 27% | `current_instances` 未配置，默认按 1 实例计算 | 补填 `current_instances: 2` |

### 📁 本次修改文件

| 文件 | 操作 | 说明 |
|------|------|------|
| `results/models/INDEX.yaml` | 修改（xinghan-ziwei-32b-v1 条目）| 新增 `online_deployment_name`、`current_instances: 2`、更新 recommendation |

### ⚠️ 未完成（下次会话跟进）

- xinghan-ziwei-32b-v1 实际缩容操作（由业务侧决策执行，Dashboard 建议已就位）
- 其余模型评估进展（guoxue Phase 3 分析、EVAL_REPORT 生成等，见上方记录）
