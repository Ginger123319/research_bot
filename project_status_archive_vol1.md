...
# 📋 会话报告 - 2026-03-20（guoxue-72b 重评 + Skill 更新）

## 📌 会话概览
- **主要目标**：使用真实线上数据重新评估 `xinghan-guoxue-72b-v1-2-reason` 的 QPS 容量，同步更新技能文档
- **模型**：xinghan-guoxue-72b-v1-2-reason（深度推理模型，avg_output_len=1386 tokens）
- **服务端点**：`https://infer-test.geniuworks.com/bazi-guoxue-eagle3-test/v1/chat/completions`
- **新 SLA**：TTFS P90 < 1.5s，E2E P90 < 180s

## ✅ 成果

### 数据处理（新路径：情况C）
- 处理 AI-data 平台 downloaded 格式（每行 list），绕过 DataConverter/DataSampler
- 产出：`datas/output_guoxue_v2/..._poisson_256_stitched.csv`（TARGET_RPM=256）
- 模型别名：`xinghan-guoxue-72b-v1-2-reason` + `八字深度`（两种写法均覆盖）
...
```
- 检查点：`logs/guoxue-v2-phase1_20260320_1524/phase1_checkpoint.md`

### Phase 2 状态（后台运行中）
- 参数：DURATION=3600s（1h/档），COOLDOWN=900s（15min）
- 初始 bracket：`LO=0.200, HI=0.312`
- 已完成轮次：第1轮 0.2498 ❌，第2轮 0.2235 ❌，第3轮 0.2114 运行中（预计继续）
- 脚本：`scripts/benchmark/run_phase2_auto_guoxue8tp_v2.sh`

### Skill 文档更新
| 文件 | 改动摘要 |
|------|---------|
| `clingo/docs/skills/traffic-dataset-prep/SKILL.md` | 新增情况C（AI-data downloaded 格式），步骤0别名探查，pandas 解析模板，DATA_FORMAT=downloaded |
| `clingo/docs/skills/qps-peak-finder/SKILL.md` | 新增 DURATION/COOLDOWN 自动推算公式，测试污染警告，production_rps 陷阱说明 |

## 🐛 关键 Bug 记录

| Bug | 症状 | 根因 | 修复 |
|-----|------|------|------|
| DataConverter 格式不兼容 | `AttributeError: 'list' object has no attribute 'get'` | downloaded JSONL 每行是 list | 绕过 DataConverter 自定义解析 |
| Phase 1 grep 失效 | 饱和判断永远走 +10，不收敛 | grep 模式不匹配 `: ` 分隔符 | 改为 `\s*[=:]\s*` 兼容格式 |
| production_rps 误配 | bracket 过早托底 4.27 收敛 | 混淆 Poisson RPM 与服务 RPS | 改为保守下限 0.05 |
| 测试污染 | 低 RPS 比高 RPS 延迟更高 | COOLDOWN=60s 不足（模型 E2E ~300s）| COOLDOWN→900s，DURATION→3600s |

## 📂 新增文件
| 文件 | 用途 |
|------|------|
| `scripts/data/process_guoxue_v2.py` | 情况C 数据处理（downloaded 格式）|
| `configs/models/xinghan-guoxue-72b-v1-2-reason/8tp_v2.env` | 新评估配置（SLA: 180s/1.5s）|
| `scripts/benchmark/run_phase1_auto_guoxue8tp_v2.sh` | Phase 1 饱和探测 |
| `scripts/benchmark/run_phase2_auto_guoxue8tp_v2.sh` | Phase 2 自适应 bracket 搜索 |
| `scripts/benchmark/run_phase3_grid_guoxue8tp_v2.sh` | Phase 3 生产网格验证 |
| `logs/guoxue-v2-phase1_20260320_1524/phase1_checkpoint.md` | Phase 1 检查点记录 |
...
```bash
# 查看 Phase 2 当前进度
tail -30 logs/phase2_guoxue8tp_v2_*.log   # 找到当前运行的日志文件

# 查看是否已收敛
grep -E "收敛|CONVERGED|ideal_rps" logs/phase2_guoxue8tp_v2_*.log | tail -10
```

**Phase 2 收敛后**：
1. 记录 `ideal_rps`
2. 更新 `run_phase3_grid_guoxue8tp_v2.sh` 中的 `IDEAL_RPS`
3. 启动 Phase 3（4档×3600s，总约4h+冷却）

### Phase 3 完成后
1. 运行 `qps-peak-finder-analysis` Skill 生成 REPORT.md
2. 在 `results/models/xinghan-guoxue-72b-v1-2-reason/` 创建 model-context.md
3. 运行 `model-eval-report` Skill 生成 EVAL_REPORT.md
4. 更新 `results/models/INDEX.yaml`

## 💡 重要经验（本次新发现）

1. **downloaded 格式识别**：`head -c 2 file.jsonl` → 输出 `[{` 即为情况C，必须绕过 DataConverter
2. **COOLDOWN 公式**：对推理模型，`COOLDOWN = max(60, avg_output_len / 2)` 秒，本模型 1386 → 693s ≈ 12min
3. **DURATION 公式**：`DURATION = max(1200, avg_output_len × 2.5)` 秒，本模型 → 3465s ≈ 1h
4. **production_rps 独立于 TARGET_PEAK_RPM**：前者来自 Grafana 实测，后者是数据集制备参数，两者不等价

## 📋 下次会话必读

### 1. 检查 guoxue-v2 Phase 2 状态
```bash
# 找到 Phase 2 日志
ls logs/ | grep guoxue-v2-phase2

# 查看收敛状态
tail -50 logs/phase2_guoxue8tp_v2_*.log
grep -E "LO_RPS|HI_RPS|ideal_rps|收敛" logs/phase2_guoxue8tp_v2_*.log | tail -20
```

### 2. Phase 2 收敛后启动 Phase 3
```bash
# 先确认 ideal_rps，然后修改 IDEAL_RPS 变量
vim scripts/benchmark/run_phase3_grid_guoxue8tp_v2.sh
# 将 IDEAL_RPS=<Phase 2 收敛值> 填入
bash scripts/benchmark/run_phase3_grid_guoxue8tp_v2.sh > logs/phase3_guoxue8tp_v2_$(date +%Y%m%d).log 2>&1 &
```
...
# 会话报告 - 2026-03-20（下午场，chart-32b 归档）

## 📌 会话概览

- **开始时间**：2026-03-20 下午（衔接上次会话，chart-32b 回放已在后台运行）
- **结束时间**：2026-03-20 傍晚
- **主要目标**：确认 chart-32b 回放测试结果 → 分析归档 → 生成最终 EVAL_REPORT.md
- **Git 分支**：main（未提交，本次为分析报告归档，无代码变更）
- **最新提交**：`8ae4a49` docs(progress): 记录 2026-03-20 下午场会话

---

## ✅ 成果

### 1. xinghan-chart-32b-v1-1-agent 全流程评估完成

本次会话是 chart-32b 评估流程的最后一环（Step 6 + Step 7），将之前所有评估结果汇聚成正式交付报告。

**回放测试结果**（118 RPM，3实例×8TP，keep-income-time 模式）：
- 成功率 99.19%（5826请求，47条超时失败）
- TTFT P90 = 0.135s（SLA 1.5s，余量91%）
- TTFS P90 = 0.442s（SLA 1.5s，余量71%）
- E2E P90 = 6.811s（SLA 150s，极宽裕）
- 单实例均摊 ~39 RPM，离 ideal_rps 457 RPM 有 **11× 余量**

**最终 EVAL_REPORT.md 核心结论**：

| 维度 | 结论 |
|------|------|
| 上线建议 | ✅ 可上线 |
| 单实例承载 | 7.61 req/s = 457 RPM（8TP × L20）|
| 单实例覆盖率 | **387%**（业务峰值 118 RPM）|
| 最小部署 | **1 实例 × 8 张 L20** |
| HA 配置 | 2 实例 × 8TP = 16 卡 |
...
| 文件 | 操作 |
|------|------|
| `results/chart-32b_replay_20260320/REPORT.md` | 🆕 新增（6节详细报告）|
| `results/chart-32b_replay_20260320/chart-32b-8tp_replay_118rpm_analysis.html` | 🆕 新增（交互分析图）|
| `results/chart-32b_replay_20260320/ttft_cdf.png` 等 7张 PNG | 🆕 新增 |
| `results/models/xinghan-chart-32b-v1-1-agent/EVAL_REPORT.md` | 🆕 新增（最终交付报告）|
| `results/models/INDEX.yaml` | ✏️ 修改（replay_* 字段 + eval_report 路径 + recommendation）|
| `results/models/xinghan-chart-32b-v1-1-agent/model-context.md` | ✏️ 追加（回放结果字段）|
| `results/README.md` | ✏️ 修改（新增 chart-32b_replay 条目）|
| `progress.md` | ✏️ 追加（本次会话完整记录）|

---

## 🐛 问题与解决方案

本次会话无新错误，均为已完成实验数据的分析归档操作。

---

## 🎯 技术决策

1. **indexed格式 Agent 流量纳入回放验证**：chart-32b 87% 的流量来自 Agent 框架（不持久化 messages），通过 `_full.csv` 分片+3列索引+DataSampler 的前置准备方案，成功将全量流量纳入回放数据集。这是一个重要的工程决策，确保了测试数据的代表性。

2. **最小部署推荐（1实例）而非保守建议**：公式计算（推荐实例数=1），但明确注明"如需HA保2实例"，给用户清晰的决策依据而不是笼统超配。

3. **资源规划参考表**：EVAL_REPORT.md 新增了"附：资源规划参考表"，涵盖当前、2.5×、4.2×、8.5× 业务增长场景下所需实例数，为未来扩容提供参考。
...
| 任务 | 模型 | 说明 |
|------|------|------|
| Phase 2 自适应逼近（运行中）| xinghan-guoxue-72b-v1-2-reason（v2 重评）| 每档1h，当前运行中；Phase 3 待完成后出图 |
| EVAL_REPORT.md | xinghan-ziwei-32b-v1 | QPS+回放均完成，但 Step 7 尚未执行 |
| EVAL_REPORT.md | tianji-querysafety-4b-v2-3 | Phase 3 剩余2档（qps7.7269/qps7.8554）未完成，REPORT.md 待更新 |
| EVAL_REPORT.md | xinghan-guoxue-72b-v1-2-reason | v2 重评流程进行中，待全流程完成 |
| 回放测试 | xinghan-guoxue-72b-v1-2-reason | 有回放数据集，但 SLA 基准已从 150s 更新为 180s，回放尚未执行 |

---

## 💡 建议与注意事项

1. **guoxue-v2 Phase 2 监控**：每档 3600s，共约 5~8 轮。下次会话优先检查收敛状态（`tail -50 logs/phase2_guoxue8tp_v2_*.log`），收敛后立即启动 Phase 3。

2. **ziwei 评估报告**：ziwei-32b 的关键问题是 **生产 RPM (100 RPM) > ideal_rps (95.4 RPM)**，SLA 在峰值时无法满足。EVAL_REPORT.md 需要明确指出扩容建议（2实例×8TP）。

3. **tianji Phase 3 补档**：qps7.7269 和 qps7.8554 两档完成后，重跑 multi_exp_compare 更新 REPORT.md §3 节。

4. **indexed 格式数据准备经验**：本次总结到 `traffic-dataset-prep/SKILL.md`，下次遇到类似 Agent 框架 indexed 数据时直接参考。

---

## 📈 下次会话计划

### 优先级 1：guoxue-v2 Phase 2/3 跟进
```bash
# 检查 Phase 2 收敛状态
ls logs/ | grep guoxue-v2-phase2
tail -50 logs/phase2_guoxue8tp_v2_*.log
...
### 优先级 2：ziwei-32b EVAL_REPORT.md
- 调用 model-eval-report Skill
- 核心结论：生产 100 RPM > SLA 上限 95.4 RPM，建议扩容至 2 实例×8TP=16卡

### 优先级 3：tianji Phase 3 补档 + REPORT.md 更新
- 检查 qps7.7269/qps7.8554 两档运行状态
- 完成后重跑 multi_exp_compare，更新 §3 节
- 最终执行 EVAL_REPORT.md

---

## 📊 整体进度快照（2026-03-22 会话结束）

| 模型 | QPS 评估 | 回放测试 | EVAL_REPORT | 状态 |
|------|---------|---------|-------------|------|
| xinghan-hepan-72b-v1-2 | ✅ 完成 | ✅ 完成 | ✅ 完成 | 🟢 已交付 |
| xinghan-chart-32b-v1-1-agent | ✅ peak-finder | ✅ 99.19% | ✅ 完成 | 🟢 已交付 |
| tianji-querysafety-4b-v2-3 | ✅（Phase3缺2档）| ✅ 100% | ⚠️ 待补档 | 🟡 基本完成 |
| xinghan-ziwei-32b-v1 | ✅ peak-finder | ✅ 历史 | ❌ 未生成 | 🟡 待Step7 |
| xinghan-guoxue-72b-v1-2-reason（4×H20）| ✅ P1+P2完成，P3运行中 | ⚠️ 待执行 | ❌ 未生成 | 🟠 进行中 |
| lingyu-235b-A22b-v9-2 | ✅ 完成 | ❌ 无数据 | ❌ 未生成 | 🟡 待Step7 |

### 关键风险提示
- ⚠️ **ziwei 生产已超载**：100 RPM > SLA 上限 95.4 RPM，峰值时 TTFS P90 超标 19%，需扩容
- ⚠️ **tianji 余量极窄**：单实例余量仅 2.6%，强烈建议扩至 9 实例
- ⚠️ **guoxue H20 4卡容量偏低**：ideal_rps = 35.3 RPM，而业务峰值 256 RPM，单实例远不够；需多实例部署
...
## 📌 会话概览
- **时间范围**：2026-03-22（续接 2026-03-20 遗留 Phase 1 运行中）
- **主要目标**：xinghan-guoxue-72b-v1-2-reason 4×H20 QPS Peak Finder Phase 1~3 全流程推进
- **Git 分支**：main（ahead 1）

## ✅ 成果

### 1. Phase 1 结果确认与 Bug 修复
- con=60 为峰值档：**1056.724 tokens/s**，max_rps_estimate = **0.4898 req/s**
- con=120 严重过载（-34.3%，2270/2400 超时），确认饱和
- 修复脚本两处 bug（详见下方问题与解决方案）

### 2. Phase 2 完成（5档，bracket 宽度 2.82%，精确收敛）
- **ideal_rps = 0.5878 req/s = 35.3 RPM**
- 首个 FAIL 点：0.6044 req/s（E2E P90 = 182.07s，超标仅 1.15%）
- Little's Law 验证通过，数据完全自洽
- 用时约 9 小时（3/20 21:59 → 3/21 03:34）

### 3. Phase 3 验证网格启动
- 4档：0.05 / 0.2293 / 0.4085 / 0.5878 req/s
- 启动时间：2026-03-22 17:59，预计完成 ~23:00

## 🔧 文件变更

| 文件 | 操作 | 说明 |
|------|------|------|
| `scripts/benchmark/run_phase1_auto_guoxue4h20.sh` | 修改 | 修复 max_rps_estimate 正则 + 新增 PEAK 追踪逻辑 |
| `logs/guoxue-v2-h20-phase1_20260320_2030/phase1_checkpoint.md` | 重写 | 人工填入正确值（0.4898 req/s） |
| `logs/guoxue-v2-h20-phase2_20260320_2053/phase2_checkpoint.md` | 自动生成 | ideal_rps=0.5878，bracket 2.82% |

## 🐛 问题与解决方案

### Bug 1：Phase 1 max_rps_estimate 正则错误（严重）
- **现象**：检查点写入 `max_rps_estimate = 694.335 req/s`、`START_RPS = 833.202 req/s`（数量级错误，正确值应为 0.4898 req/s）
- **根因**：分析脚本输出 `max_rps_estimate = A / B = C req/s`，旧正则匹配第一个数字 A（decode_throughput），而非最终值 C
- **修复**：改为三段式正则 `max_rps_estimate\s*=\s*[\d.]+\s*/\s*[\d.]+\s*=\s*\K[\d.]+`

### Bug 2：饱和时使用过载档指标（严重）
- **现象**：当 con=120 吞吐低于 con=60 时（饱和确认），脚本用 con=120 的 max_rps 写入检查点
- **根因**：脚本未追踪历史峰值，饱和确认时只有当前档数据
- **修复**：引入 PEAK_THROUGHPUT / PEAK_MAX_RPS / PEAK_CON 全程追踪，饱和时汇报峰值档

### Bug 3：Phase 2 启动时未传环境变量
- **现象**：`nohup bash run_phase2.sh` 启动后 PEAK_RPS/START_RPS 为默认值 0
- **修复**：kill 进程重启，改为 `PEAK_RPS=0.4898 START_RPS=0.5878 nohup bash ...`

## 🎯 技术决策

1. **ideal_rps 保守取 0.5878 而非尝试 0.5944**：差异仅 0.4 RPM，在测量不确定性（±5~10s E2E P90）范围内，不值得额外 1 小时测试成本

2. **Phase 2 首档即 PASS 触发大步上探（×1.25）**：0.5878 → 0.7348，因 E2E P90 裕量高达 36.8%，符合 SKILL 逻辑

3. **avg_output_len 实测 2157 vs 估算 1386**：推理模型在充足算力下生成更长的思考链；Phase 2/3 的 DURATION=3600s 仍足够（实际完成约 2117 条/h 远超稳态测量需求）

## ⚠️ 未完成事项

| 任务 | 预计时间 | 依赖 |
|------|---------|------|
| Phase 3 运行完成 | ~2026-03-22 23:00 | 无 |
| qps-peak-finder-analysis Skill → REPORT.md | Phase 3 完成后 30min | Phase 3 |
| 更新 model-context.md H20 字段 | 同上 | Phase 3 |
| 回放测试（guoxue 4×H20 or 混合方案）| 待定 | REPORT.md |
| EVAL_REPORT.md | 待定 | 回放 |

## 💡 建议与注意事项

1. **H20 4卡容量**：ideal_rps = 35.3 RPM，业务峰值 256 RPM，理论需 `ceil(256/35.3) = 8 实例`。但实际需结合混合部署（H20+L20）综合规划，不能单独用 H20 配置估算

2. **陡崖警告**：0.5878 → 0.6044 区间内，E2E P90 从 114s 跳至 182s（+58.5%），对应 RPS 仅增 2.82%。运营上需严格限流，避免接近 0.6044 req/s

3. **Phase 3 数据分析准备**：分析脚本对大 CSV（~2.3 GB/档）较慢；Phase 3 完成后用 `qps-peak-finder-analysis` Skill 自动化，不要手动逐档分析

4. **SKILL.md Bug 已记录**：本次两处 Phase 1 脚本 bug 已同步修复到 `run_phase1_auto_guoxue4h20.sh`，但 `qps-peak-finder/SKILL.md` 中的 Phase 1 模板脚本也应补充相同修复，防止下次新模型评估时复现

## 📈 下次会话计划

### 优先级 1：Phase 3 结果分析 + REPORT.md
```bash
# 检查 Phase 3 是否完成
ls logs/guoxue-v2-h20-phase3_20260322_1759/
tail -c 2000 logs/data-pipeline/guoxue_v2_h20_phase3_20260322_175900.log

# 运行 qps-peak-finder-analysis Skill 生成图表和 REPORT.md
# 参考 .cursor/skills/qps-peak-finder-analysis/SKILL.md
```

### 优先级 2：更新 model-context.md
```bash
# 填写 H20 评估结果字段
# h20_peak_decode_throughput: 1056.724 tokens/s
# h20_max_rps_estimate: 0.4898 req/s
# h20_ideal_rps: 0.5878 req/s
# h20_ideal_rpm: 35.3
# h20_sla_max_qps: 0.5878
```

### 优先级 3：回放测试规划（guoxue H20）
- 与之前的 4H20+9L20 混合回放做对比
- 确认单实例 4H20 在 256 RPM 下的表现（应严重过载）

### 优先级 4：guoxue SKILL.md Phase 1 模板补丁
- 在 `qps-peak-finder/SKILL.md` Phase 1 执行模板中补充 PEAK_* 变量和正确正则
- 避免下次新模型评估时复现同类 bug
...
## 📌 会话概览
- **时间范围**：2026-03-22（续接 3/22 早些时候启动的 H20 Phase 3）
- **主要目标**：
  1. 诊断并修复 L20 Phase 2→3 衔接失败问题
  2. 同时启动 L20 + H20 Phase 3 验证网格
  3. 通过 Brainstorming 设计并实现 Phase 2→3 自动衔接健壮性双保险
- **Git 分支**：main

## ✅ 成果

### 1. 诊断 L20 Phase 2 → Phase 3 衔接失败根因
- Phase 2 脚本（`run_phase2_auto_guoxue8tp_v2.sh`）在 Round 4（rps0.2174）准备阶段意外崩溃
- `phase2_checkpoint.md` 从未写入
- Phase 3 衔接脚本（`run_phase3_auto_guoxue8tp_v2.sh`）等待 24 小时后超时退出
- H20 Phase 2 早已收敛（ideal_rps=0.5878），但也无人触发 Phase 3

### 2. 确认 L20 ideal_rps 并手动补写 checkpoint
- rps0.2114 → E2E P90=153.19s ✅（裕量17.5%），TTFS P90=885ms ✅
- Bracket [0.2114, 0.2235] 宽度 5.4%，取保守下界 **ideal_rps = 0.2114 req/s = 12.7 RPM**
- 手动写入 `logs/guoxue-v2-phase2_20260320_1920/phase2_checkpoint.md`

### 3. 同时启动 L20 + H20 Phase 3（2026-03-22 17:54）

| 部署 | ideal_rps | 4档网格 | 预计完成 |
|------|-----------|---------|---------|
| L20 (8tp, eagle3-test) | 0.2114 req/s | 0.05/0.1038/0.1576/0.2114 | ~22:54 |
| H20 (4h20, infra) | 0.5878 req/s | 0.05/0.2293/0.4085/0.5878 | ~22:54 |

### 4. 设计并实现 Phase 2→3 自动衔接双保险机制（Brainstorming + 实现）

**双保险原理**：
- **保险 1**（Phase 2 脚本侧）：`trap EXIT` 机制，进程退出必然写 checkpoint
- **保险 2**（衔接脚本侧）：检测 Phase 2 进程已死后，自动扫描 `rps*/` 目录推断 ideal_rps

**Checkpoint 来源标记**：
| 标记 | 含义 |
|------|------|
| 无标记（正常） | bracket 收敛至 <3%，正常写入 |
| `[EXIT_TRAP]` | 脚本意外退出，以 LAST_PASS_RPS 保守取值 |
| `[PRODUCTION_FLOOR]` | 所有档 FAIL，托底至 production_rps |
| `[RECOVERY]` | 衔接脚本进程死亡后自主推断写入 |

## 🔧 文件变更

| 文件 | 操作 | 说明 |
|------|------|------|
| `logs/guoxue-v2-phase2_20260320_1920/phase2_checkpoint.md` | 新增（手动）| L20 Phase 2 收敛补档，ideal_rps=0.2114 |
| `scripts/benchmark/run_phase2_auto_guoxue8tp_v2.sh` | 修改 | +trap EXIT, +LAST_PASS_RPS 追踪, +production floor checkpoint |
| `scripts/benchmark/run_phase2_auto_guoxue4h20.sh` | 修改 | 同步 trap EXIT 机制 |
| `scripts/benchmark/run_phase3_auto_guoxue8tp_v2.sh` | 修改 | 旧方式2（目录数量判断）→ 进程死亡检测 + analyze 自主推断 |
| `clingo/docs/skills/qps-peak-finder/SKILL.md` | 修改 | +EXIT_TRAP 健壮性说明段，+常见问题新增行 |

## 🐛 问题与解决方案

| 问题 | 根因 | 解决方式 |
|------|------|---------|
| Phase 3 衔接脚本 24h 超时，Phase 3 从未启动 | Phase 2 崩溃时无 checkpoint，衔接脚本单点依赖 | 手动补写 checkpoint + 实现 EXIT trap 双保险 |
| 旧"方式2"用 `grep -l 'SLA_PASS' CSV` 误判 PASS | SLA_PASS 是 analyze 脚本的输出标记，不在 CSV 内容里 | 替换为调用 `analyze_peak_finder.py` 的正确逻辑 |
| H20 Phase 3 也未启动（无衔接脚本） | H20 配套的 Phase 3 衔接脚本未创建 | 直接带参数手动启动 `run_phase3_grid_guoxue4h20.sh` |

## 🎯 技术决策

1. **双保险策略而非单一 trap**：trap 解决"未来"问题，衔接脚本自主推断解决"已发生"问题（当前事故），两者共同确保系统对进程崩溃有弹性

2. **最少 3 次轮询后才检测进程死亡**：防止衔接脚本在 Phase 2 刚启动尚未产生任何 rps* 目录时误触发恢复逻辑

3. **取 LAST_PASS_RPS 而非 bracket 中点**：进程崩溃后 bracket 可能未完全收敛，保守取已确认 PASS 的最高档更安全

## ⚠️ 未完成事项

| 任务 | 预计时间 | 依赖 |
|------|---------|------|
| L20 + H20 Phase 3 运行完成 | ~2026-03-22 22:54 | 无（后台运行中）|
| L20 qps-peak-finder-analysis Skill → REPORT.md | Phase 3 完成后 | Phase 3 |
| H20 qps-peak-finder-analysis Skill → REPORT.md | Phase 3 完成后 | Phase 3 |
| 合并 L20 + H20 结果写入 model-context.md | 同上 | 两份 REPORT.md |
| guoxue 回放测试 | 待规划 | REPORT.md |
| EVAL_REPORT.md | 待规划 | 回放 |

## 📊 整体进度快照（2026-03-22 续会话结束）
...
| tianji-querysafety-4b-v2-3 | ✅（Phase3缺2档）| ✅ 100% | ⚠️ 待补档 | 🟡 基本完成 |
| xinghan-ziwei-32b-v1 | ✅ peak-finder | ✅ 历史 | ❌ 未生成 | 🟡 待Step7 |
| xinghan-guoxue-72b-v1-2-reason（8×L20）| ✅ P1+P2完成，**P3运行中** | ⚠️ 待执行 | ❌ 未生成 | 🟠 进行中 |
| xinghan-guoxue-72b-v1-2-reason（4×H20）| ✅ P1+P2完成，**P3运行中** | ⚠️ 待执行 | ❌ 未生成 | 🟠 进行中 |
| lingyu-235b-A22b-v9-2 | ✅ 完成 | ❌ 无数据 | ❌ 未生成 | 🟡 待Step7 |

### 关键风险提示（2026-03-23 更新）
- ⚠️ **guoxue L20 容量偏低**：ideal_rps = 12.7 RPM，业务峰值 256 RPM，单实例远不够，需多实例部署
- ⚠️ **guoxue H20 容量偏低**：ideal_rps = 35.3 RPM，同样需要多实例（理论需 ≥8 实例）
- ✅ **ziwei 已缩容建议就位**：2 实例部署，7日峰值仅 52 RPM（占 SLA 27%），Dashboard 已展示缩容至 1 实例建议（节省 8 卡 L20）
- ⚠️ **tianji 余量极窄**：单实例余量仅 2.6%，强烈建议扩至 9 实例

---

## 📋 最新会话摘要 — 2026-03-23（运维 + Dashboard 配置）

### 会话概览
- **日期**：2026-03-23
- **Git 分支**：main
- **最后提交**：`8ae4a49` docs(progress): 记录 2026-03-20 下午场会话 — 在线承载量监控集成
- **主要目标**：重启 serve.py 服务 + 接入 ziwei SGLang v0.4.6 在线监控

### 完成事项

| 任务 | 结果 |
|------|------|
| 重启 serve.py（PID 4041588，端口 18999） | ✅ 后台运行，日志 `logs/data-pipeline/serve_18999.log` |
| xinghan-ziwei-32b-v1 接入在线 RPM 监控 | ✅ `online_deployment_name: xinghan-ziwei-p32b-v1-sglang046`（v0.4.6） |
| 修正实例数为 2，更新 SLA 对比 | ✅ 7日峰值 52 RPM，SLA 利用率 27%，建议缩容至 1 实例 |

### 修改文件

| 文件 | 变更 |
|------|------|
| `results/models/INDEX.yaml` | xinghan-ziwei-32b-v1 新增 `online_deployment_name`、`current_instances: 2`、更新 recommendation |

## 📊 整体进度快照（2026-03-23 更新）
...
| tianji-querysafety-4b-v2-3 | ✅（Phase3缺2档）| ✅ 100% | ⚠️ 待补档 | ✅ 已配置 | 🟡 基本完成 |
| xinghan-ziwei-32b-v1 | ✅ peak-finder | ✅ 历史 | ❌ 未生成 | ✅ v0.4.6 已接入 | 🟡 待Step7 |
...
| lingyu-235b-A22b-v9-2 | ✅ 完成 | ❌ 无数据 | ❌ 未生成 | ✅ 已配置 | 🟡 待Step7 |

## 📈 下次会话计划

### 优先级 1：检查 guoxue Phase 3 完成状态
```bash
# L20 Phase 3
tail -20 logs/data-pipeline/guoxue_v2_h20_phase3_20260322_175900.log
...
### 优先级 2：运行 qps-peak-finder-analysis Skill 生成 REPORT.md
```bash
# L20
# Phase 1 目录: logs/guoxue-v2-eagle3-phase1_20260323_1804/
# Phase 2 目录: logs/guoxue-v2-phase2_20260320_1920/
# Phase 3 目录: logs/guoxue-v2-phase3_20260322_1754/

# H20
# Phase 2 目录: logs/guoxue-v2-h20-phase2_20260320_2053/
# Phase 3 目录: logs/guoxue-v2-h20-phase3_20260322_1754/
```

### 优先级 3：合并报告 + EVAL_REPORT.md
- 对比 L20 vs H20 容量差异
- 计算多实例部署建议（256 RPM 业务峰值所需实例数）

### 优先级 4（可选）：ziwei 缩容落地确认
- 业务侧确认后执行缩容（1 实例 → 节省 8 卡 L20）
- 缩容后更新 INDEX.yaml `current_instances: 1`
