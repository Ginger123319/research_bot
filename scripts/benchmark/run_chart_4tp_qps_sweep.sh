#!/bin/bash
# xinghan-chart-32b-v1-1-agent  QPS 拐点扫描（4TP部署）
#
...
#     curl https://infer.geniuworks.com/infra-opti-xinghan-chart-p32b-v1-agent/health
#
# 用法：
#   后台执行：nohup bash scripts/benchmark/run_chart_4tp_qps_sweep.sh \
#               > logs/data-pipeline/chart_4tp_qps_$(date +%Y%m%d_%H%M%S).log 2>&1 &
#   查看进度：tail -f logs/data-pipeline/chart_4tp_qps_<timestamp>.log
#             cat logs/chart-32b-4tp/qps_<timestamp>/progress.txt
...
echo "========================================================"
echo "xinghan-chart-32b-v1-1-agent  QPS 拐点扫描（4TP）"
echo "========================================================"
echo "  部署组:           ${GROUP_NAME}"
