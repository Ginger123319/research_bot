#!/bin/bash
# Phase 1 自动饱和探测循环 — xinghan-chart-32b-v1-1-agent 8TP
#
...
#   PEAK_RPS=X.XXXX START_RPS=X.XXXX bash scripts/benchmark/run_phase2_auto_chart8tp.sh
...
OUTPUT_BASE="${PROJECT_DIR}/logs/chart-32b-8tp-phase1_${TIMESTAMP}"
CHECKPOINT="${OUTPUT_BASE}/phase1_checkpoint.md"
mkdir -p "${OUTPUT_BASE}"

echo "========================================================"
echo "Phase 1 自动饱和探测 — chart-32b 8TP"
...
for iter in $(seq 1 ${MAX_ITERS}); do
    ...
    if [[ "${NEXT_CON}" == "SATURATED" ]]; then
        ...
        echo ""
        echo "========================================================"
        echo "Phase 1 结果汇总 — chart-32b 8TP"
        ...
        if [[ -n "${MAX_RPS}" ]]; then
            ...
            echo "    bash ${PROJECT_DIR}/scripts/benchmark/run_phase2_auto_chart8tp.sh"
        fi
        echo "  检查点: ${CHECKPOINT}"
        echo "========================================================"
        break
