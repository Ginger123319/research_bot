#!/bin/bash
# Phase 2a 自动收敛循环 — xinghan-chart-32b-v1-1-agent 8TP（Agent 数据集）
#
...
# 续跑：
#   INIT_LO=X.X INIT_HI=X.X PEAK_RPS=X.XXXX bash scripts/benchmark/run_phase2_auto_chart_agent_8tp.sh
...
# DURATION = 1800s（手动调大，保障稳态观测窗口充足）
...
OUTPUT_BASE="${PROJECT_DIR}/logs/xinghan-chart-32b-v1-1-agent/chart-32b-agent-8tp-phase2_${TIMESTAMP}"
...
# Phase 2 起始 QPS（= max_rps_estimate × 1.2，由 Phase 1 输出传入）
START_RPS="${START_RPS:-}"
...
echo "========================================================"
echo "Phase 2a 自动收敛 — chart-32b-agent 8TP"
...
for iter in $(seq 1 ${MAX_ITERS}); do
    ...
    if [[ -n "${LO_RPS}" && -n "${HI_RPS}" ]]; then
        ...
        if [[ "${CONVERGED}" == "yes" ]]; then
            ...
            echo "  IDEAL_RPS=${LO_RPS} bash ${PROJECT_DIR}/scripts/benchmark/run_phase3_grid_chart_agent_8tp.sh"
            break
        fi
