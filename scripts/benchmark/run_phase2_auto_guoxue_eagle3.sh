#!/bin/bash
# Phase 2 自适应逼近 — xinghan-guoxue-72b-v1-2-reason EAGLE3
#
# eagle3 投机解码（EAGLE3-3-1-4）；8×L20；SLA: TTFS P90 ≤ 1.5s, E2E P90 ≤ 180s
# bracket 几何中点精查，宽度 < 3% 时收敛。
#
# 用法：
#   PEAK_RPS=<peak> START_RPS=<max_rps×1.2> bash scripts/benchmark/run_phase2_auto_guoxue_eagle3.sh
...
DURATION_SECS=3600    # 1h/档（avg_output_len≈1386 → max(1200, 1386×2.5=3465) → 取 3600）
COOLDOWN_SECS=900     # 15min 冷却（E2E P90 预计 ~120~180s，需 ≥ 3× E2E 才能排空积压）
...
OUTPUT_BASE="${PROJECT_DIR}/logs/guoxue-v2-eagle3-phase2_${TIMESTAMP}"
CHECKPOINT="${OUTPUT_BASE}/phase2_checkpoint.md"
PHASE3_SCRIPT="${PROJECT_DIR}/scripts/benchmark/run_phase3_grid_guoxue_eagle3.sh"
mkdir -p "${OUTPUT_BASE}"
...
# production_rps：上线 Grafana 实测 0.195 req/s（20 实例 × 0.195 = 3.9 RPS 集群总量）
PRODUCTION_RPS=0.195
...
print(f'{(hi-lo)/lo*100:.2f}%')" 2>/dev/null || echo "—")
        ...
        cat > "${CHECKPOINT}" << EOF
# Phase 2 检查点 — guoxue-72b EAGLE3 [EXIT_TRAP]
...
echo "========================================================"
echo "Phase 2 自适应逼近 — guoxue-72b EAGLE3"
...
echo "  production_rps=${PRODUCTION_RPS}（Phase 3 网格起始 / 托底）"
...
for iter in $(seq 1 ${MAX_ITERS}); do
    ...
    if [[ -d "${LEVEL_DIR}" ]] && ls "${LEVEL_DIR}"/*.csv 2>/dev/null | grep -qv "argv"; then
        ...
        "${PYTHON}" -m llm_benchmark.benchmark.benchmark \
            --exp-name       "guoxue_eagle3_phase2_rps${RPS_TAG}" \
    ...
    # ── 收敛判断（bracket 更新后重新计算，不复用 analyze 的 NEXT_RPS）────
    if [[ -n "${LO_RPS}" && -n "${HI_RPS}" ]]; then
        CONVERGED=$(${PYTHON} -c "
...
")
        if [[ "${CONVERGED}" == "yes" ]]; then
            ...
            cat > "${CHECKPOINT}" << EOF
# Phase 2 检查点 — guoxue-72b EAGLE3 收敛
...
EOF
            ...
            echo "      bash scripts/benchmark/run_phase3_grid_guoxue_eagle3.sh"
            break
        fi

        # bracket 已建立，几何中点（bracket 更新后重算，不复用 analyze 打印值）
        CURRENT_RPS=$(${PYTHON} -c "import math; print(f'{math.sqrt(float(\"${LO_RPS}\")*float(\"${HI_RPS}\")):.4f}')")
        echo "  ▶ bracket 几何中点（已更新）: ${CURRENT_RPS}"
        ...
        if [[ "${BELOW_PROD}" == "yes" ]]; then
            ...
            cat > "${CHECKPOINT}" << EOF
# Phase 2 检查点 — guoxue-72b EAGLE3 [PRODUCTION_FLOOR]
