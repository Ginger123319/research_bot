#!/bin/bash
# Phase 2a 自动收敛循环 — tianji-querysafety-4b-v2-3 bakv1 4TP
#
# SLA 说明：该模型主判断指标为 E2E P95 ≤ 400ms。
# 因 analyze 脚本仅支持 P90 阈值，使用 E2E P90 ≤ 350ms 作为保守代理
# （基于前次 4TP 测试：QPS=8.0 时 P90=0.330s → PASS，QPS=8.5 时 P90=0.363s → FAIL，
#  与 E2E P95 ≤ 400ms 结论一致）。
#
# 用法：直接执行，脚本自动追踪 LO/HI 状态
# 续跑：INIT_LO=X.X INIT_HI=X.X bash scripts/benchmark/run_phase2_auto_tianji4tp_bakv1.sh
...
DURATION_SECS=1200    # 20min / 档
COOLDOWN_SECS=60

# ── SLA 阈值 ─────────────────────────────────────────────────
# 主判断: E2E P95 ≤ 400ms，用 E2E P90 ≤ 350ms 作保守代理
TTFS_P90_LIMIT=1.5    # 1500ms（通用基准）
E2E_P90_LIMIT=0.35    # 350ms（E2E P95 ≤ 400ms 的保守代理）

# ── Phase 1 输出（由调用方传入或修改默认值） ─────────────────
PEAK_RPS="${PEAK_RPS:-0}"
START_RPS="${START_RPS:-}"

TIMESTAMP="$(date +%Y%m%d)"
OUTPUT_BASE="${PROJECT_DIR}/logs/tianji-querysafety-bakv1-phase2_${TIMESTAMP}"
...
echo "========================================================"
echo "Phase 2a 自动收敛 — tianji-querysafety bakv1 4TP"
echo "  peak_rps=${PEAK_RPS}  start_rps=${START_RPS}"
echo "  duration=${DURATION_SECS}s  SLA: TTFS P90≤${TTFS_P90_LIMIT}s  E2E P90≤${E2E_P90_LIMIT}s"
echo "  (E2E P90 ≤ 0.35s 等效于 E2E P95 ≤ 400ms 保守代理)"
...
for iter in $(seq 1 ${MAX_ITERS}); do
    ...
    # bracket 更新后重新计算几何中点（避免复用 analyze 用旧 bracket 算出的陈旧 NEXT_RPS）
    if [[ -n "${LO_RPS}" && -n "${HI_RPS}" ]]; then
        ...
        if echo "${ANALYSIS_OUT}" | grep -q "\[NEXT_RPS=CONVERGED\]"; then
            if [[ -n "${HI_RPS}" && -n "${LO_RPS}" ]]; then
                ...
                if [[ "${WIDTH}" == "wide" ]]; then
                    echo "  ⚠️  analyze 输出 CONVERGED 但 bracket 宽度≥3%，继续 binary search"
                    NEXT_RPS=$(${PYTHON} -c "import math; print(f'{math.sqrt(float(\"${LO_RPS}\")*float(\"${HI_RPS}\")):.4f}')")
                    echo "  → 重算几何中点: ${NEXT_RPS}"
                else
                    echo ""
                    echo "🎯 Phase 2a 收敛完成！"
                    echo "   ideal_rps = ${LO_RPS} req/s  (最高确认通过档)"
