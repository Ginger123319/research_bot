#!/bin/bash
# Phase 3 上线验证网格 — xinghan-guoxue-72b-v1-2-reason 4×H20
#
# 在 [PRODUCTION_RPS, IDEAL_RPS] 区间内均匀 4 档验证 SLA 曲线
# 每档 1h，档间冷却 15min
#
# 用法：
#   IDEAL_RPS=<ideal_rps> PRODUCTION_RPS=<prod_rps> bash scripts/benchmark/run_phase3_grid_guoxue4h20.sh
...
# ── Phase 2 输出（由调用方传入）────────────────────────────────
IDEAL_RPS="${IDEAL_RPS:-}"
PRODUCTION_RPS="${PRODUCTION_RPS:-0.05}"
...
OUTPUT_DIR="${PROJECT_DIR}/logs/guoxue-v2-h20-phase3_${TIMESTAMP}"
mkdir -p "${OUTPUT_DIR}"

echo "========================================================"
echo "Phase 3 上线验证网格 — guoxue-72b V2（4×H20）"
...
for i in "${!QPS_LEVELS[@]}"; do
    ...
    if [[ -d "${LEVEL_DIR}" ]] && ls "${LEVEL_DIR}"/*.csv 2>/dev/null | grep -qv "argv"; then
        ...
        "${PYTHON}" -m llm_benchmark.benchmark.benchmark \
            --exp-name       "guoxue_v2_h20_phase3_qps${QPS_TAG}" \
