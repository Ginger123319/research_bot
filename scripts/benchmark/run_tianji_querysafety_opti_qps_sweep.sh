#!/bin/bash
# tianji-querysafety-4b-v2-3  QPS 拐点扫描（优化部署：1TP × 4DP）
#
# 对照组: run_tianji_querysafety_qps_sweep.sh（4TP × 1 实例，QPS 7.00→4.00）
# 本实验: 1TP × 4DP（4 独立副本，理论吞吐 ~4×），QPS 范围等比放大
#
# QPS 范围: 28.00 → 16.00（30 档，均等步进 ~0.414）
# 每档时长: 45min（2700s）
# 数据集:   tianji-querysafety-4b-v2-3_peak30min.csv（44,099 条，峰值 2168 RPM）
# 服务:     https://infer.geniuworks.com/infra-opti-tianji-querysafety-p4b-v23/v1/chat/completions
#
# 用法：
#   后台执行：nohup bash scripts/benchmark/run_tianji_querysafety_opti_qps_sweep.sh > /dev/null 2>&1 &
#   前台调试：bash scripts/benchmark/run_tianji_querysafety_opti_qps_sweep.sh
#   查看进度：tail -f logs/tianji_opti_qps_<timestamp>.log
#             cat logs/tianji_opti_qps_<timestamp>/progress.txt
...
SERVER_URL="https://infer-test.geniuworks.com/infra-opti-tianji-querysafety-p4b-v23/v1/chat/completions"
...
OUTPUT_DIR="${PROJECT_DIR}/logs/tianji_opti_qps_${TIMESTAMP}"
LOG_FILE="${PROJECT_DIR}/logs/tianji_opti_qps_${TIMESTAMP}.log"

# ============================================================
# QPS 档位（28.00 → 16.00，30 档，linspace 均等）
# 对应原 4TP 实验 7.00→4.00 的 4× 等比放大
# ============================================================
...
echo "========================================================"
echo "tianji-querysafety-4b-v2-3  QPS 拐点扫描（1TP × 4DP 优化部署）"
echo "========================================================"
echo "  服务 URL:         ${SERVER_URL}"
echo "  部署配置:         1TP × 4DP（4 独立副本，对照组 4TP×1 吞吐 ~4×）"
echo "  数据集:           $(basename ${DATASET_PATH})  (44,099 条，峰值 2168 RPM)"
echo "  QPS 范围:         28.00 → 16.00（${TOTAL_LEVELS} 档，步进 ~0.414）"
...
echo "qps_range=28.00_to_16.00" >> "${PROGRESS_FILE}"
echo "deploy=1TP_x_4DP" >> "${PROGRESS_FILE}"
...
for entry in "${QPS_LEVELS[@]}"; do
    ...
    LEVEL_DIR="${OUTPUT_DIR}/opti_qps_${qps}"
    mkdir -p "${LEVEL_DIR}"
...
echo ""
echo "结果目录:"
ls -1d "${OUTPUT_DIR}"/opti_qps_* 2>/dev/null | while read d; do
